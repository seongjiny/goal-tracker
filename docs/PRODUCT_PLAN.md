...
현재 구현 범위는 사용자 결정에 따라 1차와 1.1차를 함께 진행한다. 환경변수가 없을 때는 로컬 미리보기 모드, Supabase 설정 후에는 카카오 로그인과 Household 내 개인 DB 모드로 동작한다.
...
- 항목 추가
- 이름, 색상, 아이콘 설정
- 기본 아이콘 23개 제공: 기도, 노트북/개발, 운동, 기록, 반려동물, 생활 등
- 기본 색상 10개 제공
- 정렬 순서 변경
- 활성 항목 archive
- archive 항목 복원
...
- 로그인 및 서버 동기화
- 부부 초대와 같은 Household 참여(항목과 기록은 개인 분리)
- 하루 2회 이상 기록
- Daily 이외의 반복 규칙
- 알림
- 소셜 기능
...
현재는 `created_by`를 항목 소유자로 사용하고 개인 항목만 제공한다. 공동 항목은 별도 공유 범위 필드를 추가하는 후속 기능으로 둔다.

#### 보안 기준

- 사용자는 자신이 만든 항목과 기록만 읽고 변경할 수 있다.
- Household 참여 시 기존 개인 항목과 기록을 보존한다.
